...
class SUNRGBDDataset(data.Dataset):
    def __init__(self,  root_path=None,
        ...
        self.img_ids = [execute_filename(os.path.splitext(
            x)[0]) for x in sorted(os.listdir(self.color_img_path))]
    ...
    def __getitem__(self, index):
        ...
        color_img = Image.open(os.path.join(
            self.color_img_path, 'img-' + img_id) + '.jpg').convert('RGB')
        ...
        label_img = Image.open(os.path.join(
            self.label_img_path, 'img13labels-' + img_id) + '.png')
