...
class IRCADSeries(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mask_serie = [torch.Tensor(mask_serie_tf(x)).long(
        ).unsqueeze(0) // 255 for x in mask_serie]
...
class IRCADSingle(data.Dataset):
    def __init__(self, root_path=None, is_train=True):
        ...
        self.imgs_id = map(str, self.root_path.rglob(
            'PATIENT_DICOM/image_[0-9]*'))

        regex = re.compile(
            r'.*3Dircadb1.(?P<patient_id>[0-9]+)/PATIENT_DICOM/image_(?P<slice_id>[0-9]+)')
...
def another_test():
    ...
    for img, mask in dataset:
        ...
        plt.imshow(tv.utils.make_grid(mask.unsqueeze(
            1), nrow=15).squeeze().permute(1, 2, 0) * 255, alpha=0.5)
        plt.tight_layout()
        plt.show()
        plt.close()
