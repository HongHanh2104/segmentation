...
def plot_grad_flow(named_parameters, title):
    ...
    ave_grads = (ave_grads - torch.min(ave_grads)) / (torch.max(ave_grads) - torch.min(ave_grads))
    ave_grads = (max_grads - torch.min(max_grads)) / (torch.max(max_grads) - torch.min(max_grads))
